What AviScript Isn't
===============

{{#include ../links.md}}

AviScript's purpose is to provide a dynamic layer over Rust code, in the same spirit of _zero cost abstractions_.
...
* **No structures/records/tuples** &ndash; define your types in Rust instead; AviScript can seamlessly
...
* **No first-class functions** &ndash; Code your functions in Rust instead, and register them with AviScript.
...
* **No first-class closures** &ndash; do your closure magic in Rust instead:
  [turn a AviScript scripted function into a Rust closure]({{rootUrl}}/engine/call-fn.md).
...
* **No formal language grammar** &ndash; AviScript uses a hand-coded lexer, a hand-coded top-down
...
* **No bytecodes/JIT** &ndash; AviScript uses a heavily-optimized AST-walking interpreter which is fast
  enough for most real-life scenarios.

```admonish info.small "How it compares?"

See AviScript performance [benchmarks].
```
