...
The purpose of AviScript is not to be _blazing fast_, but to make it as easy and versatile as possible to
...
[performance-optimized](../start/builds/performance.md) and full builds of AviScript with [Python 3] and
[V8] ([Node.js]).

| Benchmark                                         | AviScript<br/>(Perf) | AviScript</br>(Full) | [Python 3]<br/>([bytecodes]) | [V8]<br/>([JIT]) | Description                                                                     |
| ------------------------------------------------- | :-------------: | :-------------: | :--------------------------: | :--------------: | ------------------------------------------------------------------------------- |
...
In general, AviScript is roughly 2x slower than [Python 3], which is a [bytecodes] interpreter, for
typical real-life workloads.


```admonish question "TL;DR &ndash; AviScript is usually fast enough"
...
AviScript uses [immutable strings][`ImmutableString`] to bypass cloning issues.
...
```admonish danger "DO NOT: Write the next 4D VR game entirely in AviScript"

AviScript deliberately keeps the language small and lean by omitting advanced language features
...
Avoid the temptation to write full-fledge application logic entirely in AviScript &ndash; that use case
is best fulfilled by more complete scripting languages such as JavaScript or [Lua].
```

```admonish tip "DO THIS: Use AviScript as a thin dynamic wrapper layer over Rust code"

In actual practice, it is usually best to expose a Rust API into AviScript for scripts to call.

All the core functionalities should be written in Rust, with AviScript being the dynamic _control_ layer.
...
In this case, JavaScript takes the role of AviScript while the back-end language, well... it can actually
also be Rust. Except that AviScript integrates with Rust _much_ more tightly, removing the need for
